...
public class cameraShake : MonoBehaviour {
	...
	IEnumerator Shake() {
		...
		while (shakeDuration > 0.1f) {
			...
			if(smooth)
				transform.rotation = origRot * Quaternion.Lerp(transform.localRotation, Quaternion.Euler(rotationAmount), Time.deltaTime * smoothAmount);
			else
				transform.rotation = origRot * Quaternion.Euler (rotationAmount);//Set the local rotation the be the rotation amount.
